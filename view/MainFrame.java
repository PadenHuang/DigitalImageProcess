...
public class MainFrame extends JFrame implements ListSelectionListener {
	...
	* 初始化组件*/
    private void initComponents() {
		...
		/*hwq：区域分割——效果与动态阈值分割一致*/
		regionSegmentationMethod = new JMenuItem("区域分割(A)");
		...
		regionSegmentationMethod.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e){
				...
				dlm.addElement("第"+pointer+"步：图像"+newImage+"经区域分割");
				pointer++;
			}
		});
		...
		frequencyDomainProcessing = new JMenu("图像变换(F)");
		...
		imgShow.add(morphology);//hwq:形态学合并到图像表示和描述
